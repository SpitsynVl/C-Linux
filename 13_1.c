...
int main(void) { 
    ...
    if(child_id == 0) { //работает только дочерний процесс
        printf("Child process "); 
        pid_t child = getpid(); 
        printf("PID = %d\n", child);
        sleep(60);
    } else { //работает только родительский процесс
        int status = 0; 
        if(waitpid(child_id, &status, WUNTRACED | WCONTINUED) < 0) {
            perror("waitpid"); 
            return 2; 
        }  
        while(WIFEXITED(status) == 0 && WIFSIGNALED(status) == 0) {//выходит, когда дочерний процесс завершился или получил сигнал
